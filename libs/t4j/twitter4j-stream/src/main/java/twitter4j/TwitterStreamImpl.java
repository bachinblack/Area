...
class TwitterStreamImpl extends TwitterBaseImpl implements TwitterStream {
     ...
     * @throws TwitterException when MyTwitter service or network is unavailable
     * @see twitter4j.StatusStream
     * @see <a href="https://dev.twitter.com/docs/streaming-api/methods">Streaming API Methods | MyTwitter Developers</a>
    ...
    abstract class TwitterStreamConsumer extends Thread {
        ...
        private final String NAME = "MyTwitter Stream consumer-" + (++count);
            ...
            while (!closed) {
                try {
                    ...
                    if (!closed) {
                        ...
                        if (!closed) {
                            // wait for a moment not to overload MyTwitter API
