...
public interface HttpResponseCode {
    ...
    int INTERNAL_SERVER_ERROR = 500;// Internal Server Error: Something is broken. Please post to the group so the MyTwitter team can investigate.
    int BAD_GATEWAY = 502;// Bad Gateway: MyTwitter is down or being upgraded.
    int SERVICE_UNAVAILABLE = 503;// Service Unavailable: The MyTwitter servers are up, but overloaded with requests. Try again later. The search and trend methods use this to indicate when you are being rate limited.
    int GATEWAY_TIMEOUT = 504;// The MyTwitter servers are up, but the request couldn't be serviced due to some failure within our stack. Try again later.
}
